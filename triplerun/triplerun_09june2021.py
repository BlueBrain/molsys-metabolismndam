...
REPORT_FLAG = False
NDAM_ARCHIVE_VERSION = "2021-05" # depends on which module you loaded
...
ins_glut_file_output = path_to_results + f"triplerun_ins_r_glut_{timestr}.txt"
ins_gaba_file_output = path_to_results + f"triplerun_ins_r_gaba_{timestr}.txt"
outs_glut_file_output = path_to_results + f"triplerun_outs_r_glut_{timestr}.txt"
outs_gaba_file_output = path_to_results + f"triplerun_outs_r_gaba_{timestr}.txt"

param_out_file = path_to_results + f"triplerun_param_{timestr}.txt"
um_out_file = path_to_results + f"triplerun_um_{timestr}.txt"

#####
test_counter_seg_file = path_to_results + f"triplerun_test_counter_seg0_{timestr}.txt"
wrong_gids_testing_file = path_to_results + f"triplerun_wrong_gid_errors_{timestr}.txt"
err_solver_output = path_to_results + f"triplerun_solver_errors_{timestr}.txt"

#####
#voltages_per_gid_f = "/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/in_data/voltages_per_gid.txt"
target_gids = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0.txt") # hex0 gids
exc_target_gids = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_exc_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0exc.txt")
inh_target_gids = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_inh_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0inh.txt")
target_gids_L1 = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_L1_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0l1.txt")
target_gids_L2 = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_L2_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0l2.txt")
target_gids_L3 = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_L3_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0l3.txt")
target_gids_L4 = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_L4_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0l4.txt")
target_gids_L5 = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_L5_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0l5.txt")
target_gids_L6 = np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/mc2_L6_gids.txt") #for sscx: np.loadtxt("/gpfs/bbp.cscs.ch/project/proj34/metabolismndam/sim/gids_sets/hex0l6.txt")
...
SIM_END_coupling_interval = int(SIM_END/10) # make sure it's int  #was 500.0
...
##### ATTENTION! In this script we will only consider the case when coupling between steps and ndam happens at the same tine or more frequently than between ndam and julia, i.e. julia-metabolism is outer loop. In general case (if we want steps-ndam coupling to happen less freq than jl-ndam) this needs to be refactored 
dt_nrn2dt_steps: int = SIM_END_coupling_interval #100 # can be independent variable or can be set to the same number as int(SIM_END/SIM_END_coupling_interval) so that steps-ndam and jl-ndam will happen at the same time
...
class K:
    name = 'K'
    conc_0 = 5 #3 it was 3 in Dan's example  # (mM/L) # base_conc= 2 #3 it was 3 in Dan's example #sum here is 6 which is probably too high according to Magistretti #base_conc+conc_0 is the real concentration, conc_0 is the extracellular amount simulated
    diffname = 'diff_K'
    diffcst = 2e-9
    current_var = 'ik'
    ki_var = 'ki'
...
def get_sec_mapping(ndamus, mesh):
    ...
    ## works with old NDAM 2020-12, change the line below to account for multipopulation split if using NDAM 2021-03 and more recent
    for c in ndamus.cells:   
        ...
        print("secmap: ", secmap.values())
        sys.exit("just for testing")

        tet2cellGid[int(c.CCell.gid)].append(secmap.values())  # I just need cell_gid to tet id mapping so sync julia with steps properly
...
def main():
    ...
    if NDAM_ARCHIVE_VERSION == "2020-12":
        print("using old NDAM 2020-12")
        ...
        print("using new NDAM starting from 2021-03")
    ...
    for idxm in range(int(SIM_END/SIM_END_coupling_interval)): #it was 10 before tests of 20oct2020  #it was 5 before 17aug2020 
        ...
        (neurSecmap,tet2cellGid) = get_sec_mapping(ndamus, tmgeom)
        ...
        if rank == 0 and REPORT_FLAG:
            f = open("tetConcs_nao.txt", "w+")
            fk = open("tetConcs_ko.txt", "w+")

        def fract(neurSecmap):
            # compute the currents and concentrations in ecs arising from each segment into each of the tets
            tet_currents = np.zeros((ntets,), dtype=float) #INa
            #tet_voltage = np.zeros((ntets,), dtype=float)
            tet_currents_k = np.zeros((ntets,), dtype=float) # Ik
                    ...
                    for seg, tet2fraction in zip(sec.allseg(), tet2fraction_map):
                        if tet2fraction:
                            ...
                            for tet, fract in tet2fraction:
                                ...
                                tet_currents_k[tet] += seg.ik * seg.area() * 1e-8 * fract
                                #tet_voltage[tet] += seg.v

            return(tet_currents,tet_currents_k) #,tet_voltage)

        index = np.array(range(ntets), dtype=np.uintp)
        tetConcs = np.zeros((ntets,), dtype=float) # nao
        tetConcs_k = np.zeros((ntets,), dtype=float) #ko

        # allreduce comm buffer
        tet_currents_all = np.zeros((ntets,), dtype=float)
        tet_currents_all_k = np.zeros((ntets,), dtype=float)
        ...
        for t in ProgressBar(int(SIM_END_coupling_interval / DT))(timesteps(SIM_END_coupling_interval, DT)):
            if t == int(SIM_END_coupling_interval / DT):
                print("TEST_t: ",t)
            ...
            if steps == int(SIM_END_coupling_interval / DT): #steps % dt_nrn2dt_steps == 0:
                with mt.timer.region('steps_cum'):
                    steps_sim.run( ( idxm * SIM_END_coupling_interval + t) / 1000)  # ms to sec


                with mt.timer.region('processing'):
                    (tet_currents,tet_currents_k) = fract(neurSecmap) #tet_currents = fract(neurSecmap)

                    with mt.timer.region('comm_allred_currents'):
                        comm.Allreduce(tet_currents, tet_currents_all, op=MPI.SUM)
                        comm.Allreduce(tet_currents_k, tet_currents_all_k, op=MPI.SUM)
                    ...
                    steps_sim.getBatchTetConcsNP(index, K.name, tetConcs_k)
                    # 0.001A/mA 6.24e18 particles/coulomb 1000L/m3
                    tetConcs = tetConcs + tet_currents_all * CA * tetVol
                    tetConcs_k = tetConcs_k + tet_currents_all_k * CA * tetVol
                    steps_sim.setBatchTetConcsNP(index, Na.name, tetConcs)
                    steps_sim.setBatchTetConcsNP(index, K.name, tetConcs_k)
                    if rank == 0 and REPORT_FLAG:
                        f.write(" ".join(("%e" % x for x in tetConcs)))
                        fk.write(" ".join(("%e" % x for x in tetConcs_k)))
        if rank == 0 and REPORT_FLAG:
            f.close()
            fk.close()
...
####################
        ...
        else: # for modules starting from 2021-03
            print("using new NDAM starting from 2021-03")
...
#                print("u027: ",u0[27])
            ...
            ### HERE WE NEED TO USE STEPS-simulated Kout concentration to give it as input to Julia-metabolism model
            ### but we need to either give average ecs conc among tets of given cell or iterate over tets, but then we need to change the other part of code too, 
            ### where we have per-cell properties (such as number of glut and gaba releases per presynaptic neuron, received glut and gaba events per postsyn neuron, conc and curr per neuron)

            vm[7] = u0[7] - 1.33 * (kis_mean[c_gid] - 140.0 ) # Kout
