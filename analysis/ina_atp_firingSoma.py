...
def get_atp(sim_f,subset_cells,duration_in_seconds):
    ...
    for i in range(duration_in_seconds):
        atp_mean['atp_meanconc_'+str(i+1)]  = atp_t.iloc[:,(1000*i+1):(1000+1000*i+1)].mean(axis=1) # 1000 because atpi DT = 1
...
def calc_atp_scirep(ina_t, vols_areas,Avogadro,Faraday,duration_in_seconds):
    ...
    for i in range(duration_in_seconds):
        atp_calc['atp_'+str(i+1)] = (Avogadro/Faraday)*1e-9*((combo.iloc[:,(10000*i+1):(10000+10000*i+1)].sum(axis=1))/3) # 10000 if ina DT = 0.1 in BlueConfig Report
        atp_calc['inaSum_'+str(i+1)] = combo.iloc[:,(10000*i+1):(10000+10000*i+1)].sum(axis=1)  # 10000 if ina dt = 0.1 in BlueConfig Report
