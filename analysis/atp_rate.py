...
vols_areas_f = '/gpfs/bbp.cscs.ch/project/proj34/sh_ngvm/column/other/cells_volume_calc/volumes_2020121415.txt'
...
def get_ina(sim_f,subset_cells):
    ...
    report = sim.report('NaCurrCellSum', source='h5') #NaCurrCellSumNosc NaCurrSoma NaCurrSumCol NaCurrPumpS 
    ina_data = report.get()
...
def get_atp(sim_f,subset_cells,duration_in_seconds):
    ...
    atp_mean['atp_rate']  = (atp_t.iloc[:,1550] - atp_t.iloc[:,1500])/0.05  # 1000 because atpi DT = 1
...
print("merges2out")
def merges2out(subset_cells,atp_mean):
    ...
    combo2 = pd.merge(atp_mean, subset_cells, left_on='gid',right_on='gid',how="inner")
...
merges2out(subset_cells,atp_mean)
print("finished!")
